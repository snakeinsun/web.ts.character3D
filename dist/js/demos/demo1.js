...
function start1() {
    let w = new Walker();
    let canvas = document.getElementById("twCanvas");
    trueone = new Character(canvas, 60, 45, 90);
    trueone.x = 25;
    trueone.y = 25;
    trueone.autoClearPreviousDrawing = false;
    let canvas1 = document.getElementById("twChicken");
...
C 10,0,42 R 2 -- head
    ...
    let lines = Abody
        ...
        return new DrawingLine(parseFloat(coords1[0]), parseFloat(coords1[1]), parseFloat(coords1[2]), parseFloat(coords2[0]), parseFloat(coords2[1]), parseFloat(coords2[2]));
    });
    let spheres = Abody
        .split("\n")
        .filter(l => l.trim() != "")
        ...
        let coords1 = pair[0].split(",");
        let radius = pair[1];
        return new DrawingSphere(parseFloat(coords1[0]), parseFloat(coords1[1]), parseFloat(coords1[2]), parseFloat(radius));
    });
    ...
    let canvas = document.getElementById("myCanvase");
